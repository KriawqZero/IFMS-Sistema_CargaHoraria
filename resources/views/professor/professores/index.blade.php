                  ...
                  @if (!$ehProfessorLogado)
                    <div class="flex gap-2">
                      <a class="text-gray-600 hover:text-gray-800"
                        href="{{ route('professor.professores.edit', $professor->id) }}">
                      ...
                      <div x-data="{ showModal: false }">
                        ...
                        <div x-show="showModal"
                          ...
                          <div @click.away="showModal = false" class="w-11/12 rounded-lg bg-white p-6 shadow-lg md:w-1/3">
                            <h2 class="mb-4 text-lg font-semibold text-gray-700">Tem certeza que deseja excluir esse
                              professor?
                              <span x-text="professorId"></span>
                            </h2>
                            <div class="flex justify-end space-x-2">
                              <button @click="showModal = false"
                                class="rounded bg-gray-300 px-4 py-2 text-gray-700 hover:bg-gray-400">Cancelar</button>
                              <form action="{{ route('professor.professores.destroy', ['id' => $professor->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
