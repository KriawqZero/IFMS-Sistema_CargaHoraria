<header class="flex items-center justify-between border-b-4 border-green-600 bg-white px-4 py-3 sm:px-6 sm:py-4">
  <div class="flex min-w-0 flex-1 items-center">
    ...
    <!-- Título -->
    <div class="relative mx-2 min-w-0 lg:mx-4">
      <h4 class="truncate text-lg font-medium text-gray-500 sm:text-xl lg:text-3xl">
        {{ $titulo }}
      </h4>
    </div>
  </div>
  ...
  <div class="flex items-center space-x-2 sm:space-x-4">
    ...
    <a href="{{ route('reports.index') }}"
      class="mr-2 flex items-center rounded-md bg-green-50 px-2 py-1.5 text-sm font-medium text-green-700 transition-colors hover:bg-green-100 sm:px-3 sm:py-2 md:mr-4">
      <svg class="mr-1 h-4 w-4 sm:mr-2 sm:h-5 sm:w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div x-data="{ dropdownOpen: false }" class="relative flex-shrink-0">
      <button @click="dropdownOpen = !dropdownOpen" class="flex items-center space-x-1 sm:space-x-2">
        <div class="relative block h-7 w-7 overflow-hidden rounded-full shadow sm:h-8 sm:w-8">
          <img class="h-full w-full object-cover" src="{{ asset('storage/' . $usuarioLogado->foto_src) }}"
            alt="Foto de perfil">
        </div>
        <svg class="h-4 w-4 text-gray-600 sm:h-5 sm:w-5" viewBox="0 0 16 16" fill="#9CA3AF"
