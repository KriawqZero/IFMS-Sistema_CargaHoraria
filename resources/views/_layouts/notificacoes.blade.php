...
<div x-data="{ showError: {{ $errors->any() ? 'true' : 'false' }} }"
     ...
     x-init="if (showError) { setTimeout(() => showError = false, 10000); }"
...
<div x-data="{ showSuccess: {{ session('success') ? 'true' : 'false' }} }"
     ...
     x-transition:leave-start="translate-x-0 opacity-100"
     x-transition:leave-end="translate-x-full opacity-0"
     x-init="if (showSuccess) { setTimeout(() => showSuccess = false, 10000); }"
     class="fixed top-5 right-5 z-20 inline-flex max-w-sm w-full bg-white shadow-md rounded-lg overflow-hidden">
...
<!-- Informação -->
<div x-data="{ showInfo: {{ session('info') ? 'true' : 'false' }} }"
     x-show="showInfo"
     x-transition:enter="transition ease-out duration-300 transform"
     x-transition:enter-start="translate-x-full opacity-0"
     x-transition:enter-end="translate-x-0 opacity-100"
     ...
     x-transition:leave-start="translate-x-0 opacity-100"
     x-transition:leave-end="translate-x-full opacity-0"
     x-init="if (showInfo) { setTimeout(() => showInfo = false, 10000); }"
     class="fixed top-5 right-5 z-20 inline-flex max-w-sm w-full bg-white shadow-md rounded-lg overflow-hidden">
