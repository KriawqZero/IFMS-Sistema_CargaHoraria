<div x-data="{ notificationOpen: false }" class="relative">
    <button @click="notificationOpen = !notificationOpen" class="mx-4 flex text-gray-400 focus:outline-none">
        ...
        @if ($usuarioLogado->unreadNotifications->count() > 0)
            <span
                class="absolute right-0 top-0 -translate-y-1/2 rounded-full bg-red-600 px-2 text-xs font-bold text-white">
                ...
                <form action="{{ $url }}" method="GET" class="flex w-full cursor-pointer items-center">
                    ...
                    <button type="submit"
                        ...
                        <div class="flex flex-col">
                            ...
                            <small
                                class="font-light text-gray-500">{{ $notification->created_at->diffForHumans() }}</small>
                        </div>
                    </button>
                </form>
            ...
            @if ($notifications->count() >= $this->limit)
                <div wire:loading>
                    <div class="flex justify-center">
                        <svg class="h-6 w-6 animate-spin text-green-500" xmlns="http://www.w3.org/2000/svg"
