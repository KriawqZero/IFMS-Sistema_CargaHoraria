<div x-data="{ notificationOpen: false }" class="relative">
    <button @click="notificationOpen = !notificationOpen" class="flex mx-4 text-gray-400 focus:outline-none">
        ...
        @if($usuarioLogado->unreadNotifications->count() > 0)
            <span class="absolute top-0 right-0 -translate-y-1/2 bg-red-600 text-white text-xs font-bold rounded-full px-2">
        ...
        @forelse($notifications as $notification)
            <div class="group flex items-center justify-between px-4 py-3 bg-white rounded-lg shadow hover:shadow-md transition hover:ring-2 ring-green-300">
                ...
                <form action="{{ $url }}" method="GET"
                    ...
                    @endif
                    <button type="submit" class="flex items-center w-full px-4 py-3 text-left space-x-3 {{ $notification->read_at ? 'bg-gray-100 hover:bg-gray-200' : 'bg-green-50 hover:bg-green-100' }} rounded-lg transition">
                        <!-- Imagem do usuário -->
                        <img class="object-cover w-10 h-10 rounded-full" src="{{ asset('storage/' . $notification->data['foto_src']) }}" alt="avatar">

                        <!-- Texto da notificação -->
                        <div class="flex flex-col">
                            <p class="text-sm font-medium text-gray-800">{!! $notification->data['mensagem'] !!}</p>
                            <small class="text-gray-500 font-light">{{ $notification->created_at->diffForHumans() }}</small>
                        </div>
                    </button>
                </form>

                <!-- Botão de Remover -->
                <button wire:click="removeNotification('{{ $notification->id }}')" class="ml-4 text-red-500 hover:bg-red-200 p-2 rounded-lg transition">
