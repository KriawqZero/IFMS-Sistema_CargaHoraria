    ...
    <div x-data="{ open: false }" class="border-b border-gray-200">
      <button @click="open = !open" class="w-full text-left text-lg font-medium text-gray-900">
        ...
        @if (in_array('status', $colunas))
          <span
            class="{{ $certificado->status == 'valido' ? 'bg-green-50 text-green-800' : ($certificado->status == 'pendente' ? 'bg-yellow-50 text-yellow-800' : 'bg-red-50 text-red-800') }} inline-flex rounded-full px-2 text-right text-xs font-semibold leading-5">
